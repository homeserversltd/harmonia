...
6. restarts only the selected profile's systemd user gateway;
7. proves the unit active and reads `hermes gateway status`;
8. atomically replaces `latest.json` with the run receipt.

A current installation still receives the scheduled gateway restart. Overlapping invocations return a successful `skipped-locked` receipt.
...
- `HERMES_MAINTENANCE_GATEWAY_UNIT` — explicit user gateway unit override;
