...
Slices 1-3 are landed. The atoms floor and one data registry stand; the validated-file-symlink exemplar uses `observe`, `act`, and `report-home`; and `place-file` now owns the observe-compare-write breath for each `files/converge` entry, including byte, mode, ownership, and backup movement. `files.rs` retains declaration production, its public interface, receipt shapes, aggregate accounting, and ladder behavior. Removal, symlink, hotfix backfill, all remaining bands, tools, and profile migration remain flat or at the registry status declared for their current shelf entries. No-block breath.
