...
Additive output stamps are keyed by `child.output`; a missing reference fails closed and blocks later children. Migration status is `routine-spine-landed`; managed-files/reload/conditional restart/health behavior remains owned by the service-runtime spine. `ai_coding_harness` is an operator overlay; its dissolved routine belongs in `harmonia-monad` (operator ruling 2026-08-13).
