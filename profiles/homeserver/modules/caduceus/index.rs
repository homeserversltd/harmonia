...
fn caduceus_health_with_retry(url: &str) -> CmdResult {
...
fn managed_files(
            ...
            if apply {
                if let Some(parent) = path.parent() {
                    fs::create_dir_all(parent)
                        .map_err(|e| format!("caduceus-managed-file-parent-failed: {e}"))?;
                }
                fs::write(&path, file.content.as_bytes())
                    .map_err(|e| format!("caduceus-managed-file-write-failed: {e}"))?;
                fs::set_permissions(
                    &path,
                    fs::Permissions::from_mode(file.mode.unwrap_or(0o644)),
                )
                .map_err(|e| format!("caduceus-managed-file-mode-failed: {e}"))?;
    ...
    write_json(
        &receipt_dir.join("caduceus-managed-files.json"),
        &json!({
            "schema": "harmonia.homeserver.caduceus_managed_files.v1",
            "ok": ok,
            "module": module.id,
            "missing": missing,
            "written": written,
            "apply": apply,
            "changed": changed,
            "first_missing_signal": if ok { "none" } else { "caduceus-managed-file-missing" }
        }),
    )?;
    Ok(outcome)
}
...
fn install_binary(
    ...
    if !apply {
        ...
        return Ok(OperationOutcome {
            ...
            changed: false,
            skipped: true,
            message: "caduceus binary install planned".to_string(),
            command: None,
        });
    }
    ...
    write_command_receipt(receipt_dir, "caduceus-service-stop", &stop)?;
    let tmp_install = install_bin.with_extension("harmonia-new");
    fs::copy(artifact, &tmp_install).map_err(|e| format!("caduceus-artifact-copy-failed: {e}"))?;
...
fn ensure_service_active(
    ...
    if !apply {
        ...
        return Ok(OperationOutcome {
            ...
            changed: false,
            skipped: true,
            message: "caduceus service activation planned".to_string(),
            command: None,
        });
    }
    let daemon_reload = command_capture("/usr/bin/systemctl", &["daemon-reload"]);
    write_command_receipt(receipt_dir, "caduceus-daemon-reload", &daemon_reload)?;
    if !daemon_reload.ok {
        return Ok(OperationOutcome {
            ok: false,
            changed: false,
            skipped: false,
            message: "caduceus systemd daemon-reload failed".to_string(),
            command: None,
        });
    }
    let enable = command_capture("/usr/bin/systemctl", &["enable", "--now", service]);
    write_command_receipt(receipt_dir, "caduceus-service-enable", &enable)?;
    let active = command_capture("/usr/bin/systemctl", &["is-active", service]);
    write_command_receipt(receipt_dir, "caduceus-service-active", &active)?;
    Ok(OperationOutcome {
        ok: enable.ok && active.ok,
        changed: enable.ok,
        skipped: false,
        message: "caduceus service enabled".to_string(),
        command: None,
    })
}
...
fn write_run_receipt(
    ...
    write_json(
        ...
        &json!({
            "schema": "harmonia.homeserver.caduceus_public_lever.v1",
            "ok": ok,
            "changed": changed,
            "mutation": apply,
            "repo": repo,
...
pub(crate) fn execute(
    ...
    if !git_outcome.ok {
        write_run_receipt(
            ...
            false,
            git_outcome.changed,
            "caduceus-source-git-artifact-failed",
            repo,
            branch,
            &source_dir,
            None,
        ...
        return Ok(ModuleExecution::from_operations(
            vec![(
                ...
                OperationOutcome {
                    ...
                    skipped: false,
                    message: "caduceus source sync failed".to_string(),
                    command: None,
                },
            )],
            &module.id,
    ...
    write_command_receipt(receipt_dir, "caduceus-source-sha", &source_sha)?;
            ...
            vec![
                (
                    ...
                    OperationOutcome {
                        ...
                        changed: git_outcome.changed,
                        skipped: false,
                        message: "caduceus source planned".to_string(),
                        command: None,
                    },
                ),
                ("caduceus-managed-files", managed),
            ],
            &module.id,
        ));
    }
        ...
        return Ok(ModuleExecution::from_operations(
            vec![(
                ...
                OperationOutcome {
                    ...
                    changed: false,
                    skipped: false,
                    message: "caduceus source sha missing".to_string(),
                    command: None,
                },
            )],
            &module.id,
    ...
    write_command_receipt(receipt_dir, "caduceus-cargo-build", &build)?;
    if !build.ok {
        write_run_receipt(
            receipt_dir,
            apply,
        ...
        return Ok(ModuleExecution::from_operations(
            vec![(
                "caduceus-cargo-build",
                OperationOutcome {
                    ok: false,
                    changed: false,
                    skipped: false,
                    message: "caduceus cargo build failed".to_string(),
                    command: None,
                },
            )],
            &module.id,
    ...
    let artifact = source_dir.join("target/release/caduceus");
    ...
    if !install.ok {
        write_run_receipt(
            ...
            false,
            install.changed,
            "caduceus-binary-install-failed",
            repo,
            branch,
            &source_dir,
            Some(&source_sha_value),
        ...
        return Ok(ModuleExecution::from_operations(
            vec![("caduceus-binary-install", install)],
            &module.id,
        ));
    }
    ...
    fs::write(&source_sha_file, format!("{source_sha_value}\n"))
        .map_err(|e| format!("caduceus-source-sha-write-failed: {e}"))?;

    let service_outcome = ensure_service_active(receipt_dir, service, apply)?;
    let health = caduceus_health_with_retry(health_url);
    write_command_receipt(receipt_dir, "caduceus-health", &health)?;
    ...
    let first_missing_signal = if ok {
        ...
        "caduceus-service-not-active"
    } else {
        "caduceus-health-failed"
    };
    let changed =
    ...
    println!("schema=harmonia.homeserver.caduceus_public_lever.v1");
    println!("ok={ok}");
        ...
        vec![
            (
                ...
                OperationOutcome {
                    ...
                    changed: git_outcome.changed,
                    skipped: false,
                    message: "caduceus source synced".to_string(),
                    command: None,
                },
            ),
            ("caduceus-managed-files", managed),
            ("caduceus-binary-install", install),
            ("caduceus-service", service_outcome),
            (
                "caduceus-health",
                OperationOutcome {
                    ok: health.ok,
                    ...
                    message: if health.ok {
                        "caduceus HTTP health proved".to_string()
                    } else {
                        "caduceus HTTP health failed".to_string()
                    },
                    command: None,
                },
            ),
