...
pub(crate) const ID: &str = "systemd";
    ...
    for file in &module.template_files {
        ...
        if let Some(mode) = file.mode {
            if !(0o400..=0o777).contains(&mode) {
                return Err(format!("systemd-mode-rejected-{}", file.target));
            }
        }
    }
    Ok(())
...
fn render_templates(
    ...
    fs::create_dir_all(&render_dir).map_err(|e| format!("systemd-render-dir-failed: {e}"))?;
    ...
    for file in &module.template_files {
        ...
        let raw = fs::read_to_string(&source).map_err(|e| {
            format!(
                "systemd-template-read-failed {}: {e}",
                source.display()
            )
        })?;
        ...
        fs::write(&out, body.as_bytes())
            .map_err(|e| format!("systemd-render-write-failed {}: {e}", out.display()))?;
        if let Some(mode) = file.mode {
            fs::set_permissions(&out, fs::Permissions::from_mode(mode))
                .map_err(|e| format!("systemd-render-mode-failed {}: {e}", out.display()))?;
        }
        rendered.push(json!({"source": file.source, "target": file.target, "rendered": out, "mode": file.mode}));
    }
    let ok = missing.is_empty();
    write_json(
        &receipt_dir.join("systemd-render.json"),
        &json!({
            "schema": "harmonia.homeserver.systemd_files.render.v1",
            "ok": ok,
            "module": module.id,
            ...
            "first_missing_signal": if ok { "none" } else { "homeserver-systemd-template-missing" }
        }),
    )?;
    Ok(OperationOutcome {
        ok,
        ...
        skipped: false,
        message: format!(
            "rendered {} systemd file templates",
...
fn install_rendered(
        ...
        if apply {
            if let Some(parent) = target.parent() {
                fs::create_dir_all(parent).map_err(|e| {
                    format!(
                        "systemd-target-parent-failed {}: {e}",
                        parent.display()
                    )
                })?;
            }
            ...
            let desired = fs::read(&rendered).map_err(|e| {
                format!(
                    "systemd-render-readback-failed {}: {e}",
                    rendered.display()
                )
            })?;
            if before.as_deref() != Some(desired.as_slice()) {
                ...
                fs::write(&tmp, &desired).map_err(|e| {
                    format!("systemd-target-write-failed {}: {e}", tmp.display())
                })?;
                if let Some(mode) = file.mode {
                    fs::set_permissions(&tmp, fs::Permissions::from_mode(mode)).map_err(|e| {
                        format!("systemd-target-mode-failed {}: {e}", tmp.display())
                    })?;
                }
                fs::rename(&tmp, &target).map_err(|e| {
                    format!(
                        "systemd-target-promote-failed {}: {e}",
                        target.display()
                    )
                })?;
    ...
    let ok = missing.is_empty();
    write_json(
        &receipt_dir.join("systemd-install.json"),
        &json!({
            "schema": "harmonia.homeserver.systemd_files.install.v1",
            "ok": ok,
            "module": module.id,
            "apply": apply,
            "planned": planned,
            ...
            "missing": missing,
            "changed": changed,
            "first_missing_signal": if ok { "none" } else { "homeserver-systemd-render-missing" }
        }),
    )?;
    Ok(OperationOutcome {
        ok,
        ...
        skipped: !apply,
        message: if apply {
            "converged HOMESERVER systemd files".to_string()
        } else {
            "planned HOMESERVER systemd files".to_string()
        },
        command: None,
    })
}
...
fn validate_absolute_target(target: &str) -> Result<(), String> {
    ...
    if !path.is_absolute() || target.contains("..") {
        return Err(format!("systemd-target-rejected-{target}"));
    }
    let allowed = target.starts_with("/etc/systemd/system/");
    if !allowed {
        return Err(format!("systemd-target-rejected-{target}"));
    }
    Ok(())
...
fn render_template(
    ...
    for (key, value) in variables {
        if !safe_variable_value(value) {
            return Err(format!("systemd-variable-value-rejected-{key}"));
        }
