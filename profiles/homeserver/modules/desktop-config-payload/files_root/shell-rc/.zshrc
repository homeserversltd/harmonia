...
# Source enhanced less configuration if available
if [ -f ~/.enhanced_less_config ]; then
    source ~/.enhanced_less_config
fi
_TMUX_LOAD=true
...
export PATH="/usr/local/go/bin:/usr/local/sbin:/usr/sbin:/sbin:/usr/bin:/bin:$HOME/bin:$HOME/.local/bin:$PATH"
...
# Create persistent FIFO for nnn plugins if it doesn't exist
...
# Only source atuin env if it exists
