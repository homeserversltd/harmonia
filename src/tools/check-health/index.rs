...
pub(crate) fn proof_battery(
    ...
    crate::write_command_receipt(request.receipt_dir, "proof-explain", &explain)?;
    operations += 1;
    if !explain.ok {
        return Ok((
            false,
    ...
    if manifests.is_empty() {
        ...
        crate::write_command_receipt(request.receipt_dir, "proof-validate-ladder", &missing)?;
        operations += 1;
        return Ok((
            false,
    ...
    for (index, manifest) in manifests.iter().enumerate() {
        ...
        crate::write_command_receipt(request.receipt_dir, &name, &validate)?;
        operations += 1;
        if !validate.ok {
            return Ok((
                false,
    ...
    crate::write_command_receipt(request.receipt_dir, "proof-plan-run", &plan)?;
    operations += 1;
    if !plan.ok {
        return Ok((
            false,
