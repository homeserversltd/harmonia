...
fn act(spec: &Spec, before: &Observation) -> Result<(bool, Value), String> {
    ...
    let signer_ca_fingerprint = response
        .get("ca_fingerprint")
        .and_then(Value::as_str)
        .ok_or("tls-lifecycle-signer-ca-fingerprint-missing")?;
    ...
    if signer_ca_fingerprint != root_fingerprint {
        return Err("tls-lifecycle-root-fingerprint-mismatch".into());
    }
    let root = fs::read(&spec.root).map_err(|error| error.to_string())?;
    ...
    let mut chain = leaf.as_bytes().to_vec();
    chain.extend_from_slice(&root);
    atomically_write(&spec.chain, &chain, 0o644)?;
