...
pub(crate) fn report(log: &Path, verdict: crate::atoms::ask::aur_package::Verdict, outcome: &crate::OperationOutcome) -> Result<(), String> {
    let message = if verdict == crate::atoms::ask::aur_package::Verdict::UpstreamMovedPastPin { format!("verdict={}; nudge=bless-new-pin", verdict.as_str()) } else { format!("verdict={}; outcome={}", verdict.as_str(), outcome.message) };
...
pub(crate) fn attest_artifact_lock(log: &Path, observation: &crate::atoms::ask::aur_package::ArtifactLockObservation) -> Result<(), String> {
