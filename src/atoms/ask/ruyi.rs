...
fn register(
    ...
    let stored = match curl(&put, Some(&bytes)) {
        Ok(x) => match parse_put(&x) {
            Ok(v) => v,
            Err(e) if e == "ruyi-stored-row-malformed" => return Err(e),
            Err(e) => {
    ...
    let staves = match curl(&get, None) {
        Ok(x) => parse_get(&x)?,
        Err(x) => {
            return Ok(RuyiReceipt {
                schema: REGISTER_SCHEMA.into(),
...
pub(crate) fn ruyi_receipt_from_beam(
    ...
    let row = validate_row(RuyiRow {
        ...
        harmonia_sha: lock.minted_from.harmonia_sha,
...
pub(crate) fn fetch_roster_receipt() -> Result<Value, String> {
    ...
    parse_get(&raw)?;
    serde_json::from_str(&raw).map_err(|_| "ruyi-roster-malformed".to_string())
}
#[cfg(test)]
mod tests {
    use super::*;
